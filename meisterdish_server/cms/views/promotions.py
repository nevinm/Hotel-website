...
from libraries import custom_error, json_response
from meisterdish_server.models import GiftCard, PromoCode, User
...
def manage_gift_card(request, data, user):
    ...
    try:
        if "edit_id" in data and str(data["edit_id"]).strip() != "":
            ...
            action = "Updat"
            if GiftCard.objects.exclude(pk=data["edit_id"]).filter(Q(
                    code=code) | Q(name=name)).exists():
                return custom_error(
                    "Either Gift Card code or name already exists.")
        ...
        return json_response({
            "status": 1,
            "id": gc.id,
            "message": action + "ed " + name,
            "code": code})

    except Exception as error:
        log.error("Manage GiftCards : " + error.message)
...
def list_gift_cards(request, data, user):
    ...
    try:
        ...
        try:
            ...
            log.error("Gift Card list pagination : " + error.message)
        ...
        for gc in gcs:
            gc_list.append({
                ...
                "email": gc.email,
                "message": gc.message,
                "user": gc.user.first_name.title(
                ) + " " + gc.user.first_name.title(),
                "code": gc.code,
                "amount": gc.amount
            })
        # End format response
        ...
        log.error("Failed to list Gift Cards." + error.message)
...
def manage_promocode(request, data, user):
    ...
    try:
        if "edit_id" in data and str(data["edit_id"]).strip() != "":
            ...
            action = "Updat"
            if PromoCode.objects.exclude(
                    pk=data["edit_id"]).filter(code=code).exists():
                return custom_error(
                    "The Promo code (" + code + ") already exists.")
            elif GiftCard.objects.filter(code=code).exists():
                return custom_error(
                    "A gift card with this code or name already exists.")
        ...
        return json_response({
            "status": 1,
            "id": promo.id,
            "message": action + "ed " + code,
            "code": code})

    except Exception as error:
        log.error("Manage Promo codes : " + error.message)
        return custom_error("An error has occurred. Please try again later.")
...
def list_promocodes(request, data, user):
    ...
    try:
        ...
        try:
            ...
            log.error("Promocode list pagination : " + error.message)
        ...
        log.error("Failed to list promo codes." + error.message)
...
def change_promocode_status(request, data, user):
    '''
    API to change promocode status from admin
    :param request:
    :param data:
    :param user:
    '''
    try:
        ...
        return json_response({
            "status": 1,
            "message": msg + " promo code " + code_obj.code,
            "new_status": int(status)})
    except Exception as error:
        log.error("Failed change promo code status." + error.message)
...
def manage_credits(request, data, user):
    '''
    API to manage credits of a particular user
    :param request:
    :param data:
    :param user:
    '''
