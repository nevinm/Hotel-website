    ...
    def handle(self, *args, **options):
        try:
            ...
            orders = Order.objects.filter(
                status__lt=3, delivery_type="delivery",
                delivery_time__year=now.year,
                delivery_time__month=now.month, delivery_time__day=now.day,
                delivery_time__hour=now.hour + 2)
            ...
            self.stdout.write('Error : "%s"' % error.message)
        else:
            self.stdout.write(
                'Successfully sent %d notifications out of %d' % (
                    sent, orders.count()))
    ...
    def send_sms_notification(self, order):
        try:
            ...
            txt = "Your Meisterdish is getting prepared and will be delivered \
                within the next hour. Please make sure that you are \
                at location to receive your meal"
            ...
            log.error(
                "Failed to send order SMS to : " + number + " : " +
                error.message)
            return False
